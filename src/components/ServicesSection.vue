    ...
    <div class="mx-auto flex max-w-[1140px] flex-col gap-12 px-7 lg:flex-row lg:items-center lg:gap-16">
      ...
      <div class="flex-1 grid grid-cols-1 gap-[14px] sm:grid-cols-2">
        <template v-for="(card, i) in displayItems" :key="card?.id ?? i">
          <article v-if="loaded" class="group relative overflow-hidden rounded-xl border border-[var(--border-40)] dark:border-[rgba(126,165,224,0.14)] p-[22px] transition-all duration-300 hover:-translate-y-[5px] hover:border-[var(--bg-accent-40)] dark:hover:border-[rgba(126,165,224,0.3)]"
          ...
          >
            ...
            <div class="mb-4 grid h-11 w-11 place-items-center rounded-xl border border-[var(--bg-accent-30)] dark:border-[rgba(126,165,224,0.3)] text-sis-accent [&_svg]:h-[22px] [&_svg]:w-[22px]" style="background: rgba(47,155,224,0.1)">
          ...
          <div v-else class="rounded-xl border border-[var(--border-40)] dark:border-[rgba(126,165,224,0.14)] p-[22px]">
            <Skeleton class="mb-4 h-11 w-11 rounded-xl" />
