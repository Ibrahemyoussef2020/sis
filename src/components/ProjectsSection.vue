  ...
  >
    <div class="w-[calc(100%-64px)] max-w-[1240px] mx-auto relative z-10">
      ...
      >
        ...
        <template v-else>
          <div class="lg:border-r border-[var(--border-30)] dark:border-[rgba(126,165,224,0.14)] flex lg:flex-col overflow-x-auto lg:overflow-visible">
            <div v-for="i in 5" :key="i" class="flex items-center gap-3 px-5 min-h-[82px] border-b border-[var(--border-30)] dark:border-[rgba(126,165,224,0.14)] last:border-b-0">
              <Skeleton class="w-[19px] h-[19px] rounded flex-shrink-0" />
        ...
        <template v-else>
          <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,0.55fr)_minmax(300px,0.45fr)] min-w-0">
            <div class="p-[48px_38px] space-y-5">
              <Skeleton class="h-3 w-24 rounded-full" />
              <Skeleton class="h-10 w-64 rounded-lg" />
              <Skeleton class="h-5 w-full rounded-full" />
              <Skeleton class="h-5 w-11/12 rounded-full" />
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-[28px] pt-[27px]">
                <div class="space-y-2">
                  <Skeleton class="h-3 w-28 rounded-full mb-[5px]" />
                  <Skeleton class="h-3 w-20 rounded-full" />
                  <Skeleton class="h-3 w-24 rounded-full" />
                  <Skeleton class="h-3 w-16 rounded-full" />
                </div>
                <div class="space-y-2">
                  <Skeleton class="h-3 w-24 rounded-full mb-[5px]" />
            ...
            <div class="relative min-h-[340px] lg:min-h-[530px]">
      ...
      <div class="mt-24 pt-16 border-t border-[var(--border-30)] dark:border-[rgba(126,165,224,0.14)]">
        ...
        <template v-else>
          <div class="flex flex-wrap gap-2 mb-7">
            <Skeleton v-for="i in 5" :key="i" class="h-8 w-20 rounded-lg" />
          </div>
        </template>
        ...
        <template v-else>
          <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <div v-for="i in 6" :key="i" class="rounded-xl border border-[var(--border-30)] dark:border-[rgba(126,165,224,0.14)] p-6 flex flex-col gap-4" style="background: var(--bg-card-2)">
              <div class="flex items-start justify-between gap-4">
                <Skeleton class="w-[45px] h-[45px] rounded-xl flex-shrink-0" />
                <Skeleton class="h-3 w-20 rounded-full" />
              </div>
              <div class="flex items-center justify-between gap-4">
                <Skeleton class="h-3 w-24 rounded-full" />
              ...
              <div class="flex flex-wrap gap-[6px] pt-4 border-t border-[var(--border-20)] dark:border-[rgba(126,165,224,0.08)]">
                <Skeleton v-for="j in 4" :key="j" class="h-5 w-14 rounded-md" />
              </div>
            </div>
          </div>
        </template>

        <div v-if="loaded && activeProjectSector === 'All' && !showAllProjects" class="mt-8 text-center">
          <button
            type="button"
...
const { industries, projects, loaded } = storeToRefs(siteStore);
